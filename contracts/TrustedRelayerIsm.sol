 ...
 * @notice Simplest ISM variant that only allows a single trusted relayer
 *         (mailbox processor) to deliver Hyperlane messages.
 *
 * This is identical to the PingPong example ISM but placed under
 * `privatetransfer/` so the private transfer routers can declare their own ISM
 * without importing example code.
 */
